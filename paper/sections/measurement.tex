...
\paragraph{Aggregate metrics hide per-prediction drift.}
...
\paragraph{Churn is not a recoding of accuracy.}
...
benchmarks --- harder tasks show more churn.  But the methods that
most reduce churn are not the methods that most improve accuracy:
deep ensembles raise mean accuracy without reducing churn, and
twin-bootstrap cuts churn at a small accuracy cost
(Section~\ref{sec:experiments}, Table~\ref{tab:main}).
...
A \emph{canonical} seed determines a single train/test split; within
...
We run $\nCanonicalSeeds$ independent replicates of the protocol at
canonical seeds $\canonicalSeed$, $7$, and $42$ (each replicate is a
different train/test split with its own ten train seeds), for both
the chemistry-MLP comparison and the pretrained-backbone scope
checks (GIN, ChemBERTa, Waterbirds).  Magnitudes quoted in prose are
across-replicate means; the development hyperparameter-selection
rule (Section~\ref{sec:methods}) picks the same value on every
replicate independently (Appendix~\ref{app:seed_sensitivity}).
...
five; per-dataset filter outcomes are in Appendix~\ref{app:repro}.  Featurisation is
