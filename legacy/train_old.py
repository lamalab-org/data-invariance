...
def make_dataloaders(cfg: DictConfig) -> dict[str, DataLoader]:
    ...
    if cfg.dataset.name == "cmnist":
    ...
    elif cfg.dataset.name in ("mof_thermal", "mof_solvent", "perovskite", "battery"):
        ppath = cfg.dataset.parquet_path
        ...
        train_ds = ChemistryDataset(
            parquet_path=ppath, target_column=target_col,
            split="train", seed=seed,
            spurious_property=spur_prop, spurious_correlation=spur_corr,
        )
        id_test_ds = ChemistryDataset(
            parquet_path=ppath, target_column=target_col,
            split="test", seed=seed,
        )
        ood_test_ds = ChemistryDataset(
            parquet_path=ppath, target_column=target_col,
        ...
        data_dir = cfg.dataset.data_dir
        train_ds = CelebADataset(split="train", data_dir=data_dir)
        id_test_ds = CelebADataset(split="val", data_dir=data_dir)
        ood_test_ds = CelebADataset(split="test", data_dir=data_dir)

    elif cfg.dataset.name in ("bace", "bbbp", "hiv", "clintox", "esol"):
        from data_molnet import MolNetDataset
        data_dir = getattr(cfg.dataset, "data_dir", "./data/molnet")
        seed = cfg.training.seed
        train_ds = MolNetDataset(name=cfg.dataset.name, split="train", seed=seed, data_dir=data_dir)
        id_test_ds = MolNetDataset(name=cfg.dataset.name, split="test", seed=seed, data_dir=data_dir)
        ood_test_ds = MolNetDataset(name=cfg.dataset.name, split="test_scaffold", seed=seed, data_dir=data_dir)

    elif cfg.dataset.name == "civilcomments":
    ...
    kwargs = dict(
        ...
        persistent_workers=True if _use_cuda else False,
    )
    return {
...
def train_random_split(
    ...
    for epoch in range(cfg.training.epochs):
        ...
        for batch in loaders["train"]:
            x = _to_device(batch["image"], device)    # (B, 3, 28, 28)
...
def discover_environments(
    ...
    if existing_model is not None:
        ...
        if cfg.dataset.arch == "resnet":
            # Freeze backbone during discovery — faster and sufficient for loss scoring.
        ...
        for ep in range(n_epochs_disc):
            ...
            if ep >= n_epochs_disc - avg_window:
                ...
                with torch.no_grad():
                    for batch in loaders["train"]:
                        x_b = _to_device(batch["image"], device)
    ...
    if criterion == "permutation":
        ...
        with torch.no_grad():
            for batch in loaders["train"]:
                x = _to_device(batch["image"], device)   # (B, C, H, W)
                idx = batch["index"]
